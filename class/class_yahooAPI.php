                                    ...
                                    if($data['type'] == 'open'){
                                          if(isset($data['attributes']) && is_array($data['attributes'])){
                                                foreach($data['attributes'] as $k => $d){
                                                      switch ($k) {
                                                      // クエリにマッチした検索結果数
                                                      case 'TOTALRESULTSAVAILABLE':      $datalist['TOTALRESULTSAVAILABLE']  = $d;      break;
                                                      // 実際に返却された検索結果数
                                                      case 'TOTALRESULTSRETURNED':      $datalist['TOTALRESULTSRETURNED']  = $d;      break;
                                                      // 返却された検索結果の先頭の順位
                                                      case 'FIRSTRESULTPOSITION':            $datalist['FIRSTRESULTPOSITION']  = $d;      break;
                                                      }
                                                }
                                          }
                                    }
