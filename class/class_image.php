...
//-----------------------------
      ...
      function createImageFromFile($filename,$type=NULL,$dx=0,$dy=0,$dw=0,$dh=0){
            ...
            if($type != ""){
                  switch($type){
                  case 'image/jpeg':
                  case 'image/pjpeg':   $ext = 'jpeg';          break;
      ...
      */
      ...
      function processingImage($dest,$src,$dst_x,$dst_y,$src_x,$src_y,$dst_w,$dst_h,$src_w,$src_h){
            ...
            // リサイズ処理(荒いがはっきり)
            case C_IMAGE_PRO_RESIZE:    return ImageCopyResized($dest,$src,$dst_x,$dst_y,$src_x,$src_y,$dst_w,$dst_h,$src_w,$src_h);    break;
            // サンプリング処理(きれいだがぼける)
            case C_IMAGE_PRO_RESAMPLE:  return ImageCopyResampled($dest,$src,$dst_x,$dst_y,$src_x,$src_y,$dst_w,$dst_h,$src_w,$src_h);  break;
            default:
      ...
      function createImageFromFileEX($dest,$src,$type_string,$src_type=NULL,$x=0,$y=0,$w=0,$h=0){
            ...
            switch($type){
            // フィット(縦横の比率を保持)
            case 'F':      $success = $this->resizeImage($dest,$src,$width,$height,$src_type,false,$x,$y,$w,$h); break;
            // トリミング(縦横の比率を保持しない)
            case 'T':      $success = $this->trimmingImage($dest,$src,$width,$height,$src_type,false,$x,$y,$w,$h); break;
            // 縮小(縦横の比率を保持しない)
            case 'S':      $success = $this->resizeImage($dest,$src,$width,$height,$src_type,true,$x,$y,$w,$h); break;
            }
            return $success;
      }
        function createImageFromBinaryEX($filename,$binary,$type_string,$x=0,$y=0,$w=0,$h=0){
                ...
                if($src_image){
                        ...
                        switch($type){
                        // フィット(縦横の比率を保持)
                        case 'F':      $resource = $this->resizeImageFromResource($src_image,$width,$height,false,$x,$y,$w,$h); break;
                        // トリミング(縦横の比率を保持しない)
                        case 'T':      $resource = $this->trimmingImageFromResource($src_image,$width,$height,false,$x,$y,$w,$h); break;
                        // 縮小(縦横の比率を保持しない)
                        case 'S':      $resource = $this->resizeImageFromResource($src_image,$width,$height,true,$x,$y,$w,$h); break;
                        }
                        // メモリ開放
