...
// キャリア判別
//+++++++++++++++++++++++++++++
function get_agent($agent=NULL){ $agent = ($agent ? $agent : $_SERVER['HTTP_USER_AGENT']); }
function is_mobile_docomo_agent($agent=NULL){ return is_user_agent(array("/^DoCoMo/i"),get_agent($agent)); }
function is_mobile_kddi_agent($agent=NULL){ return is_user_agent(array("/^KDDI\-/i","/UP\.Browser/i"),get_agent($agent)); }
function is_mobile_softbank_agent($agent=NULL){ return is_user_agent(array("/^(J\-PHONE|Vodafone|MOT\-[CV]|SoftBank)/i"),get_agent($agent)); }
function is_mobile_willcom_agent($agent=NULL){ return is_user_agent(array("/^PDXGW/i","/(DDIPOCKET|WILLCOM)/i"),get_agent($agent));  }
function is_iphone_agent($agent=NULL){ return is_user_agent(array("/iPhone/i"),get_agent($agent)); }
function is_ipad_agent($agent=NULL){ return is_user_agent(array("/iPad/i"),get_agent($agent)); }
function is_ipod_agent($agent=NULL){ return is_user_agent(array("/iPod/i"),get_agent($agent)); }
function is_android_agent($agent=NULL){ return is_user_agent(array("/Android/i"),get_agent($agent)); }
function is_android_mobile_agent ($agent=NULL) { if (!is_android_agent($agent)) { return false; } return is_user_agent(array("/Mobile/i"),get_agent($agent)); }
function is_android_tablet_agent ($agent=NULL) { if (!is_android_agent($agent)) { return false; } return (is_android_mobile_agent($agent) ? false : true); }
function is_tablet_agent ($agent=NULL) { return (is_ipad_agent($agent) || is_android_tablet_agent($agent)); }
